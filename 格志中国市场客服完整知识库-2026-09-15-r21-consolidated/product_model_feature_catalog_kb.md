...
6. 如果资料已明确确认某个型号或 SKU 支持官方 `速印通/Grozziie` 手机 App，则该 App 能在 Android 和 iOS/iPhone 上运行，可直接回答该型号或 SKU 支持这两个手机系统。App 在应用商店可下载只证明 App 的系统可用性；仍须先有该型号或 SKU 支持 App 的证据，不能仅凭蓝牙、Wi-Fi、型号名或 App 可下载就推断机器兼容。
