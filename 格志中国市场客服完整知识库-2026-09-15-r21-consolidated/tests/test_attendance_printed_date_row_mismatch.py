...
def test_structured_date_row_mismatch_card_requires_model_before_steps() -> None:
    ...
    assert "只有客户选择第三方卡微调" in card["model_policy"]
...
def test_generic_print_position_card_checks_tht_card_before_machine_adjustment() -> None:
    ...
    assert card["auto_reply_allowed"] is True
    assert "原装考勤卡" in card["reply_template"]
    assert "15组和09组" in card["reply_template"]
    assert "09组最晚05:59" in card["reply_template"]
    assert "按住1号/+增加键" in card["reply_template"]
    assert "13组" not in card["reply_template"]
