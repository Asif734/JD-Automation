...
def test_global_policy_source_contains_all_approved_rules() -> None:
    ...
    required = [
        ...
        "用户视频的画面和音频信息足够确定问题与处理路径时，也不触发本原则",
        "简短复述客户关注点",
        "视频证据充分时可直接提供步骤",
        "只有不同型号或平台确实需要不同方案",
        "安全提醒",
...
def test_training_principles_reference_single_clarification_policy() -> None:
    ...
    assert "视频画面和音频足以确定问题时直接提供分步方案" in text
...
def test_structured_global_policy_is_safe_and_cross_platform() -> None:
    ...
    assert card["block_branch_guidance_before_confirmation"] is False
    assert card["video_evidence_exception"] is True
