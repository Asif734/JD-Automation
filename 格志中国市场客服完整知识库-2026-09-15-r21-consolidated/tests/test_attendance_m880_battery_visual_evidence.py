...
def test_structured_battery_visual_card_is_model_specific_and_safe() -> None:
    ...
    assert "M880D" in card["models"]
    assert "T960D" in card["models"]
    assert "M880DA" in card["models"]
    assert "M880DT" in card["models"]
    assert card["auto_reply_allowed"] is False
    assert card["model_facts"]["M880D"] == "内置备用电池"
    assert card["model_facts"]["T960D"] == "内置备用电池"
    assert card["model_facts"]["M880DA/M880D-A/880D-A"] == "同一型号别名，内置备用电池"
    assert card["model_facts"]["M880"] == "无内置备用电池"
