...
from tutorial_video_catalog import match_tutorial
...
def build_catalog_media_reply(
    ...
    *,
    ...
    """Build an answer-first sequence with at most one exact catalog video."""
    if not intro.strip():
        raise ValueError("必须先提供简短客服说明")
    ...
    if not tutorial:
        return [{"type": "text", "content": intro.strip()}]
    validate_video(tutorial["platform"], tutorial["video_url"])
    return [
        {"type": "text", "content": intro.strip()},
        {
            "type": "video",
            "content": tutorial["video_url"],
