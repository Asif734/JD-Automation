...
def detect_product_line(query: str) -> str | None:
    ...
    if any(t in q for t in ["针打", "针式", "td630", "th880", "票据"]) and any(
        t in q for t in ["wifi", "wi-fi", "蓝牙", "app", "ip"]
    ):
...
def search(index: dict, query: str, top_k: int = 3, include_source_chunks: bool = False) -> list[dict]:
    ...
    if q in hf and hf[q] in cards_by_id:
        # Exact curated mappings are deterministic routes, not a weak hint.
        # Keep enough headroom that broad risk/keyword matches cannot outrank
        # the explicitly approved card for the same complete query.
        scores[hf[q]] += 10_000
    ...
    risk_query = q.replace("ip地址", "").replace("mac地址", "")
    for risk_term in HIGH_RISK_TERMS:
        if normalize(risk_term) in risk_query:
...
def main() -> None:
    ...
    if args.json:
        # ASCII-escaped JSON remains valid UTF-8 and is stable when this CLI is
        # captured from Windows processes using different active code pages.
