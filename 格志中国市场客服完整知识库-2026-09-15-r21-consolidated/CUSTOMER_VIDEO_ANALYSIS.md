...
视频证据包生成 `audio.wav` 后，先从画面或客户说明确认操作，再运行同操作的描述性参考匹配：

```bash
.venv-video/bin/python scripts/dot_matrix_audio_matcher.py match outputs/customer_video_analysis/<case>/audio.wav --operation normal_printing
```

可用操作标签：`normal_printing`、`front_paper_manual_pullout`、`front_paper_feed`、`printer_power_on`、`rear_paper_feed`。

不提供 `--operation` 时，工具返回 `operation_context_required`，不会猜测操作。当前每种操作只有一个已确认正常种子参考，因此工具固定返回描述性相似度以及 `normality_decision: not_determined`、`fault_diagnosis: not_determined`，不得把相似度直接写成正常、异常或具体故障。
