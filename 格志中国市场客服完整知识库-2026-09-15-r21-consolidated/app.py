...
from tutorial_video_catalog import TutorialCatalogError, enrich_reply_with_tutorial
...
def has_detailed_attendance_context(message: str, product: str | None = None) -> bool:
    """True only for the ordinary paper-card manifest, not specialized catalogs."""
    text = normalize_text(" ".join(part for part in [product or "", message] if part))
    if any(term in text for term in [
        "smart_attendance_machine",
        "智能考勤机",
        "bluetooth_attendance_machine",
        "蓝牙考勤机",
        "dot_matrix",
        "needle_printer",
        "针式打印机",
        ...
        return False
    return has_attendance_context(message, product)
...
def quick_answer(message: str, product: str | None = None) -> dict | None:
    ...
    if "色带" in compact and any(term in compact for term in ["多久", "多长", "多少字符", "多少次", "打卡多少", "打多少", "能用"]):
        return quick_result("quick_answer_ribbon_life_need_product_line", "亲，您问的是考勤机色带还是新款针式打印机色带呀？两种不能混用；新款针式打印机色带通用。", auto_reply_allowed=False)
...
def add_relevant_tutorial(
    result: dict,
    message: str,
    platform: str | None,
    product_line: str | None,
) -> dict:
    """Append one exact catalog tutorial without replacing the written answer."""
    if not platform or result.get("video_url"):
        return result
    try:
        enriched = enrich_reply_with_tutorial(
            str(result.get("reply") or ""),
            message,
            platform,
            product_line=product_line,
        )
    except TutorialCatalogError:
        return result
    if not enriched["tutorial"]:
        return result
    updated = dict(result)
    updated["reply"] = enriched["reply"]
    updated["actions"] = [*result.get("actions", []), *enriched["actions"]]
    updated["tutorial"] = enriched["tutorial"]
    updated["sources"] = list(dict.fromkeys([
        *result.get("sources", []),
...
def build_reply(
    ...
    attendance_context = has_detailed_attendance_context(message, product)
    ...
    if platform and attendance_context:
        try:
    ...
    if quick_reply:
        return add_relevant_tutorial(quick_reply, message, platform, product)
    ...
    return add_relevant_tutorial(
        result,
        message,
        platform,
        top.get("product_line") or product,
    )
