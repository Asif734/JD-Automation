...
- `china_market_video_catalog.json`：京东、天猫/千牛、拼多多的跨产品教程目录；回复时只选当前平台唯一严格匹配链接。
- `dot_matrix_audio_reference_profiles.json`：用户确认正常的五类针式打印机操作声音种子特征；必须先确定操作，只用于描述性匹配。
...
检索到文字答案后，再用 `tutorial_video_catalog.py` 检查是否存在同产品线、同主题、同系统、同平台教程。存在时把一个首选链接附在文字答案后；不存在、待重传、已隔离或有歧义时保持文字答案，不跨平台补链。
